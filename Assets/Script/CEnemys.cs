...
public class CEnemys : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        switch (storyID)
        {
            case 1:
                ...
                if (timer <= 0)
                {
                    rad = Random.Range(0, 1);
                    transform.position = Vector2.MoveTowards(transform.position, new Vector2(Mathf.Sin(rad)*5 , transform.position.y), 1.0f);
            ...
            case 2:
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (i % 4 == 0)
                    {
                        ...
                        GameObject.Find("Bullet02(Clone)").SendMessage("initCount");
                    }
                }
                GameObject.Find("Bullet02(Clone)").SendMessage("initCount");
                break;
